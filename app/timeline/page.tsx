...
const stages = [
  ...
  {
    no: "02", short: "风控信保", title: "确认客户是否具备合作条件", phase: "客户准入", owner: "外贸服务管家 · 资金部 · 法务风控", time: "T+1 / 人工尽调约 2 周", color: "#b97824",
    customer: "提交企业资质、近 4 个月出运计划及在手订单信息，等待额度审批。",
    team: ["归集营业执照、法人证件及经营资料", "信保通新建或检索企业买方代码", "提交限额、账期与支付方式并跟进审批"],
    departments: [{name:"外贸服务管家",action:"向客户收集全套企业资质、近 4 个月出运计划及在手订单信息"},{name:"资金部",action:"在信保通提交限额申请并持续跟进审批"},{name:"法务风控",action:"参与异常企业的人工尽调与风险判断"},{name:"企微机器人小花",action:"限额批复后自动发送批复结果，资产运营自此节点开始介入"}],
    handoff: "管家收集材料 → 资金部申报跟进 → 中信保反馈额度 → 小花自动推送批复结果",
  ...
  },
  {
    ...
    team: ["按企业规模、融资金额和红线匹配资方", "同步资方准入结果并组织合同签署", "签署后发起企微审批，由相关负责人逐级批复并由法务完成风险卡控"],
    departments: [{name:"资产运营部",action:"匹配资金方、集中提报并批复准入结果"},{name:"资金方",action:"依据准入标准审核企业资质"},{name:"外贸服务管家",action:"拿到准入结果、组织合同签署并在签署后发起企业微信审批"},{name:"相关负责人",action:"按企业微信审批链完成业务及管理层批复"},{name:"法务",action:"终审合同风险；担保合同重点核验达到规定比例的股东签字等担保条件"},{name:"资产运营部",action:"审批完成后进行合同编号、登记与归档"}],
    handoff: "资产运营提报 → 资方批复 → 管家组织签署 → 企微逐级审批 → 法务终审 → 资产运营归档",
    document: ["资方准入批复", "物流框架合作协议", "担保合同及股东签署材料", "企业微信审批记录", "业务合同档案台账"], tools: ["亿海融", "E签宝", "企业微信"], result: "合同审批完成并归档",
  },
  {
    no: "04", short: "注册建联", title: "授信配置落地，服务团队就位", phase: "准入落地", owner: "资产运营部 · 管家 · 履约服务部", time: "签约通过后", color: "#3e8f8c",
    customer: "完成公众号注册并签署三方总确权协议，加入专属服务群。",
    team: ["依据限额批复配置融资方案、保险及已准入资金方", "校验授信额度、账期、支付方式与有效期并完成系统落地", "引导法人注册确权、填写交接单并建立专属服务群"],
    departments: [{name:"资产运营部",action:"依据限额批复配置服务方案、保险及已准入资金方；核对授信额度、账期、支付方式、有效期与资方规则"},{name:"外贸服务管家",action:"收到配置结果后，引导法人注册、签署三方总确权协议并填写客户交接单"},{name:"履约服务部",action:"接收交接信息，创建客户及供应商资料，为首票业务做好履约准备；建立企微服务群，同步补贴与服务规则并持续维护客户关系"}],
    handoff: "限额批复推送 → 资产运营完成授信与系统配置 → 管家引导注册确权 → 履约服务部接收客户并持续服务",
    document: ["保险限额批复", "融资与授信方案配置", "三方总确权协议", "客户交接单"], tools: ["亿海融", "公众号", "企业微信"], result: "授信配置落地，具备业务接入条件",
  },
  {
    ...
    team: ["物流系统建单并生成托书等单据", "信保投保，执行资产校验与融资校验", "资方审核授信及贸易材料后完成融资放款，结算部向货代供应商支付费用"],
    departments: [{name:"外贸企业",action:"出货并提交报关单、预估费用等材料"},{name:"履约服务部",action:"物流建单并生成托书等业务单据"},{name:"资产运营部",action:"登记订单、投保审单并发起融资"},{name:"资金方",action:"审核授信与贸易材料，完成融资放款"},{name:"结算部 / 财务部",action:"组织对账并向货代供应商支付费用"}],
    handoff: "客户交单 → 履约建单制单 → 资产运营投保审单 → 资方放款 → 结算部付款",
    document: ["报关单 / 托书", "费用通知单", "投保申报记录"], tools: ["物流 SaaS", "亿海融", "信保通"], result: "资方放款、供应商收款",
  },
  {
    no: "06", short: "回款闭环", title: "应收对账，资料完成归档", phase: "单票执行", owner: "客户 · 履约服务部 · 财务部 · 结算部 · 资产运营部", time: "账期届满前后", color: "#173f57",
    customer: "收到履约客服的还款提醒，确认费用确认单并按期完成回款。",
    team: ["履约客服提醒还款并开展应收对账", "履约服务部生成费用确认单并推送客户确认，财务开具应收发票", "客户回款后上传保后材料，资产运营审核并推送资方"],
    departments: [{name:"履约服务部",action:"提前提醒还款日期及应付总金额，开展应收对账，生成费用确认单并推送客户确认；跟进回款并归集、上传提单、费用确认单及应收发票"},{name:"财务部",action:"根据确认结果开具应收发票，并确认客户回款"},{name:"结算部",action:"粘贴货代发票水单并维护相关付款凭证"},{name:"资产运营部",action:"审核费用确认单、发票和提单等保后材料，审核通过后推送资方"}],
    handoff: "履约客服提醒 → 应收对账 → 客户确认费用确认单 → 财务开票 → 客户回款 → 履约上传保后材料 → 资产运营审核推送资方",
    document: ["费用确认单", "应收发票", "提单"], tools: ["亿海融", "企业微信", "物流 SaaS"], result: "保后材料完成推送，单笔业务闭环",
  },
];
...
const interactionGroups = [
  { group: "前端获客", departments: [
    ...
    { name: "业务拓展部", relation: "贷前材料、注册与签约", topics: [
      ...
      { title: "合同签约协同", detail: "管家组织合同签署，并在签署后通过企业微信发起审批；相关负责人按审批链逐级批复，法务完成终审与风险卡控。担保合同需重点核验达到规定比例的股东签字等担保条件，审批通过后由资产运营编号归档。", output: "合同及担保材料 · 企微审批记录 · 法务终审结果" },
  ...
  { group: "金融与职能", departments: [
    { name: "资金部", relation: "资方、放款与资金风险", topics: [
      ...
      { title: "回款与资方还款", detail: "资金部同步企业回款信息；资产运营完成保后材料审核与推送后，资金部负责后续资方本金归还及结果反馈。", output: "回款信息 · 资方还款结果" },
...
const verifiedInteractions = [
  { group: "授信阶段", departments: [
    { name: "业务拓展部", relation: "外贸服务管家的申请、材料与客户现场协同", topics: [
      { title: "限额申请", detail: "管家提交企业限额申请资料；若中信保批复额度不足或未通过，则向客户说明并按实际情况申请提额或调整业务安排。", output: "限额申请资料 · 客户沟通结果" },
      { title: "合同审批与法务终审", detail: "合同签署后由管家在企微发起审批，相关负责人按审批链逐级批复，法务作为风险卡控环节完成终审。担保合同同步核验达到规定比例的股东签字等担保条件，审批完成后由资产运营编号并归档。", output: "合同审批单 · 担保材料 · 法务终审结果 · 合同号" },
  ...
  { group: "资产运营部内", departments: [
    ...
    { name: "资产运营岗", relation: "放款、异常推进与流程优化", topics: [
      { title: "放款与保后进度跟进", detail: "持续监控放款流程状态；贷后阶段审核履约上传的保后材料，审核通过后推送资方并跟进反馈。", output: "放款状态 · 保后审核与推送记录" },
